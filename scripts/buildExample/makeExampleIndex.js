...
exports.makeExampleIndex = function(ex, examples) {
    ...
    ex.index =
        ex.index.slice(0, insertionPoint) + exampleHeaderStub + ex.index.slice(insertionPoint);
    ...
    if (chartIndex < examples.length - 1) {
        header.select('li.next-arrow a').property('href', '../' + examples[chartIndex + 1].dir);
    } else {
        header.select('li.next-arrow').classed('disabled', true);
    }

    if (chartIndex > 0) {
        header.select('li.back-arrow a').property('href', '../' + examples[chartIndex - 1].dir);
    } else {
